...
import se.hig.oodp.b9.u1.bonus.Ellipse;
...
public class TestEllipse
{
    ...
    {
        assertTrue("Position is off", e.getCenter()
                .dist(new Vertex2D(2.0, 1.0)) <= T.PRECISION);
        assertTrue("Width is off", Math.abs(e.getWidth() - 1.0) <= T.PRECISION);
        assertTrue("Height is off",
                Math.abs(e.getHeight() - 0.7) <= T.PRECISION);
    }
    ...
    public void testScale()
    {
        ...
        assertTrue("Height is off",
                Math.abs(e.getHeight() - 0.847) <= T.PRECISION);
    }
}
